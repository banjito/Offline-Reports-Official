...
interface FormData {
  ...
  jobNumber: string;
  technicians: string;
  temperature: { fahrenheit: number; celsius: number; tcf: number; humidity: number; correctionFactor: number };
  substation: string;
  eqptLocation: string;
...
const initialFormData: FormData = {
  ...
  temperature: { fahrenheit: 68, celsius: 20, tcf: 1, humidity: 0, correctionFactor: 1 },
  substation: '', eqptLocation: '', status: 'PASS',
  nameplate: {
    manufacturer: '', kvaBase: '', kvaCooling: '', voltsPrimary: '', voltsPrimarySecondary: '',
    voltsSecondary: '', voltsSecondarySecondary: '', connectionsPrimary: 'Delta', connectionsSecondary: 'Wye',
...
const TwoSmallDryTypeXfmrMTSReport: React.FC<ReportProps> = ({ reportData, onSave, job }) => {
  ...
  useEffect(() => {
    ...
    setFormData(prev => ({
      ...prev,
      temperature: { ...prev.temperature, tcf: newTcf, correctionFactor: newTcf }
    }));
  ...
  const loadFromProps = (data: any) => {
    ...
    // This report uses report_data JSONB column
    // When synced down, the report_data content is flattened into the top level
    // So we need to check both: data.report_data (if not flattened) and data directly (if flattened)
    const rd = data.report_data || data;
    
    console.log('=== Loading Two Small Dry Type Xfmr MTS Report ===');
    console.log('Raw data received:', JSON.stringify(data, null, 2).substring(0, 500));
    console.log('Using rd:', JSON.stringify(rd, null, 2).substring(0, 500));
    
    // Now extract nested objects from the report_data
    const np = rd.nameplate || data.nameplate || {};
    const ig = rd.indicatorGauges || rd.indicator_gauges || data.indicatorGauges || data.indicator_gauges || {};
    const ir = rd.insulationResistance || rd.insulation_resistance || data.insulationResistance || data.insulation_resistance || {};
    const tr = rd.turnsRatio || rd.turns_ratio || data.turnsRatio || data.turns_ratio || {};
    const te = rd.testEquipment || rd.test_equipment || data.testEquipment || data.test_equipment || {};
    const temp = rd.temperature || data.temperature || {};

    console.log('Extracted nameplate:', np);
    console.log('Extracted indicatorGauges:', ig);
    console.log('Extracted insulationResistance:', ir);
    console.log('Extracted turnsRatio:', tr);
    console.log('Extracted testEquipment:', te);

    setFormData({
      customer: rd.customer || data.customer || prev.customer,
      address: rd.address || data.address || prev.address,
      user: rd.user || data.user || prev.user,
      date: rd.date || data.date || prev.date,
      identifier: rd.identifier || data.identifier || prev.identifier,
      jobNumber: rd.jobNumber || rd.job_number || data.jobNumber || data.job_number || prev.jobNumber,
      technicians: rd.technicians || data.technicians || prev.technicians,
      temperature: {
      ...
      substation: rd.substation || data.substation || prev.substation,
      eqptLocation: rd.eqptLocation || rd.eqpt_location || data.eqptLocation || data.eqpt_location || prev.eqptLocation,
      status: rd.status || data.status || prev.status,
      nameplate: {
        ...
        voltsPrimarySecondary: np.voltsPrimarySecondary || np.volts_primary_secondary || prev.nameplate.voltsPrimarySecondary,
        voltsSecondary: np.voltsSecondary || np.volts_secondary || prev.nameplate.voltsSecondary,
        voltsSecondarySecondary: np.voltsSecondarySecondary || np.volts_secondary_secondary || prev.nameplate.voltsSecondarySecondary,
      ...
      indicatorGauges: {
        liquidLevel: ig.liquidLevel || ig.liquid_level || prev.indicatorGauges.liquidLevel,
        temperature: ig.temperature || prev.indicatorGauges.temperature,
        pressureVacuum: ig.pressureVacuum || ig.pressure_vacuum || prev.indicatorGauges.pressureVacuum
      },
      visualInspectionItems: ensureArray(
        rd.visualInspectionItems || rd.visual_inspection_items || data.visualInspectionItems || data.visual_inspection_items,
        prev.visualInspectionItems
      ),
      visualInspectionComments: rd.visualInspectionComments || rd.visual_inspection_comments || data.visualInspectionComments || prev.visualInspectionComments,
      insulationResistance: {
        tests: ensureArray(ir.tests, prev.insulationResistance.tests),
        dielectricAbsorptionAcceptable: ir.dielectricAbsorptionAcceptable || ir.dielectric_absorption_acceptable || prev.insulationResistance.dielectricAbsorptionAcceptable
      },
      turnsRatio: {
      ...
      comments: rd.comments || data.comments || prev.comments
    });
  };
  ...
  return (
    <div className="report-container">
      ...
      <div className="report-header">
        <h1 className="report-title">Two Small Dry Type Transformer MTS Report</h1>
            ...
            <tr>
              ...
              <td>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  <span>/</span>
                  <input type="text" value={formData.nameplate.voltsPrimarySecondary} onChange={(e) => handleInputChange('nameplate.voltsPrimarySecondary', e.target.value)} className="table-input" style={{ width: '60px' }} />
                </div>
              </td>
              <td>
            ...
            <tr>
              ...
              <td>
                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                  ...
                  <span>/</span>
                  <input type="text" value={formData.nameplate.voltsSecondarySecondary} onChange={(e) => handleInputChange('nameplate.voltsSecondarySecondary', e.target.value)} className="table-input" style={{ width: '60px' }} />
                </div>
              </td>
              <td>
                <div style={{ display: 'flex', gap: '8px' }}>
      ...
      <div className="report-section">
        ...
        <table className="data-table">
          <tbody>
            <tr>
              <td className="label-cell">Liquid Level:</td>
              <td><input type="text" value={formData.indicatorGauges.liquidLevel} onChange={(e) => handleInputChange('indicatorGauges.liquidLevel', e.target.value)} className="table-input" /></td>
              <td className="label-cell">Temperature:</td>
              <td><input type="text" value={formData.indicatorGauges.temperature} onChange={(e) => handleInputChange('indicatorGauges.temperature', e.target.value)} className="table-input" /></td>
              <td className="label-cell">Pressure/Vacuum:</td>
              <td><input type="text" value={formData.indicatorGauges.pressureVacuum} onChange={(e) => handleInputChange('indicatorGauges.pressureVacuum', e.target.value)} className="table-input" /></td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="report-section">
            ...
            <tbody>
              {formData.insulationResistance.tests.map((test, index) => (
                <tr key={index}>
                  ...
                  <td><input type="text" value={test.dielectricAbsorption} onChange={(e) => handleInsulationChange(index, 'dielectricAbsorption', e.target.value)} className="table-input" /></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <table className="data-table" style={{ marginTop: '16px' }}>
          <tbody>
            <tr>
              <td className="label-cell">Dielectric Absorption Acceptable:</td>
              <td>
                <select value={formData.insulationResistance.dielectricAbsorptionAcceptable} onChange={(e) => handleInputChange('insulationResistance.dielectricAbsorptionAcceptable', e.target.value)} className="table-input" style={{ width: '100px' }}>
...
export default TwoSmallDryTypeXfmrMTSReport;
