...
const createDefaultFormData = () => ({
  ...
  dielectricWithstandTests: Array(6).fill(null).map((_, i) => ({
    busSection: `Bus ${i + 1}`,
    values: { ag: '', bg: '', cg: '' },
    testVoltage: '',
    unit: 'µA'
...
const SwitchgearReport: React.FC<SwitchgearReportProps> = ({ job, reportData, onSave }) => {
  ...
  useEffect(() => {
    if (reportData) {
      ...
      if (!merged.insulationResistanceTests?.length) merged.insulationResistanceTests = createDefaultFormData().insulationResistanceTests;
      if (!merged.temperatureCorrectedTests?.length) merged.temperatureCorrectedTests = createDefaultFormData().temperatureCorrectedTests;
      if (!merged.contactResistanceTests?.length) merged.contactResistanceTests = createDefaultFormData().contactResistanceTests;
      if (!merged.dielectricWithstandTests?.length) merged.dielectricWithstandTests = createDefaultFormData().dielectricWithstandTests;
      setFormData(merged);
    }
  }, [reportData]);
  ...
  const updateDielectricTest = (index: number, field: string, value: any) => {
    const tests = [...formData.dielectricWithstandTests];
    if (field.startsWith('values.')) {
      const key = field.split('.')[1];
      tests[index] = { ...tests[index], values: { ...tests[index].values, [key]: value } };
    } else {
    ...
    setFormData((prev: any) => ({ ...prev, dielectricWithstandTests: tests }));
  };
    ...
    <div className="report-container">
      ...
      <section className="report-section">
        ...
        <div className="form-grid-6">
          <div className="form-field"><label>Customer:</label><input value={formData.customer || formData.customerName || ''} onChange={e => setField('customer', e.target.value)} readOnly={!isEditing} className="report-input" /></div>
          <div className="form-field"><label>Address:</label><input value={formData.address || formData.customerLocation || ''} onChange={e => setField('address', e.target.value)} readOnly={!isEditing} className="report-input" /></div>
      ...
      <section className="report-section">
        ...
        <div className="table-container">
          <table className="report-table ir-table">
            <thead>
              <tr><th>Bus</th><th>A-G</th><th>B-G</th><th>C-G</th><th>A-B</th><th>B-C</th><th>C-A</th><th>A-N</th><th>B-N</th><th>C-N</th><th>Test V</th><th>Unit</th></tr>
            </thead>
            <tbody>
              {formData.insulationResistanceTests.map((test: any, idx: number) => (
                <tr key={idx}>
                  ...
                  <td><select value={test.testVoltage} onChange={e => updateInsulationTest(idx, 'testVoltage', e.target.value)} disabled={!isEditing} className="report-input"><option value="">-</option>{TEST_VOLTAGE_OPTIONS.map(v => <option key={v} value={v}>{v}</option>)}</select></td>
                  <td><select value={test.unit} onChange={e => updateInsulationTest(idx, 'unit', e.target.value)} disabled={!isEditing} className="report-input">{IR_UNITS.map(u => <option key={u.symbol} value={u.symbol}>{u.symbol}</option>)}</select></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="report-section">
        ...
        <div className="table-container">
          <table className="report-table ir-table">
            <thead>
              <tr><th>Bus</th><th>A-G</th><th>B-G</th><th>C-G</th><th>A-B</th><th>B-C</th><th>C-A</th><th>A-N</th><th>B-N</th><th>C-N</th><th>Unit</th></tr>
            </thead>
            <tbody>
      ...
      <section className="report-section">
        ...
        <div className="table-container">
          <table className="report-table contact-table">
            <thead>
              <tr><th>Bus</th><th>A Phase</th><th>B Phase</th><th>C Phase</th><th>Neutral</th><th>Ground</th><th>Unit</th></tr>
            </thead>
            <tbody>
              {formData.contactResistanceTests.map((test: any, idx: number) => (
                <tr key={idx}>
                  ...
                  <td><select value={test.unit} onChange={e => updateContactTest(idx, 'unit', e.target.value)} disabled={!isEditing} className="report-input">{CONTACT_RESISTANCE_UNITS.map(u => <option key={u.symbol} value={u.symbol}>{u.symbol}</option>)}</select></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="report-section">
        ...
        <div className="table-container">
          <table className="report-table">
            <thead>
              <tr><th>Bus</th><th>A-G</th><th>B-G</th><th>C-G</th><th>Test V</th><th>Unit</th></tr>
            </thead>
            <tbody>
              {formData.dielectricWithstandTests.map((test: any, idx: number) => (
                <tr key={idx}>
                  <td>{test.busSection}</td>
                  <td><input value={test.values.ag} onChange={e => updateDielectricTest(idx, 'values.ag', e.target.value)} readOnly={!isEditing} className="report-input" /></td>
                  <td><input value={test.values.bg} onChange={e => updateDielectricTest(idx, 'values.bg', e.target.value)} readOnly={!isEditing} className="report-input" /></td>
                  <td><input value={test.values.cg} onChange={e => updateDielectricTest(idx, 'values.cg', e.target.value)} readOnly={!isEditing} className="report-input" /></td>
                  <td><input value={test.testVoltage} onChange={e => updateDielectricTest(idx, 'testVoltage', e.target.value)} readOnly={!isEditing} className="report-input" /></td>
                  <td><input value={test.unit} onChange={e => updateDielectricTest(idx, 'unit', e.target.value)} readOnly={!isEditing} className="report-input" /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="report-section">
        ...
        <div className="equipment-grid">
          ...
          <div className="equipment-row">
            ...
            <input value={formData.testEquipment?.lowResistance?.name || ''} onChange={e => setField('testEquipment.lowResistance.name', e.target.value)} readOnly={!isEditing} className="report-input" placeholder="Name" />
            <label>Serial #:</label>
            <input value={formData.testEquipment?.lowResistance?.serialNumber || ''} onChange={e => setField('testEquipment.lowResistance.serialNumber', e.target.value)} readOnly={!isEditing} className="report-input" />
            <label>AMP ID:</label>
            <input value={formData.testEquipment?.lowResistance?.ampId || ''} onChange={e => setField('testEquipment.lowResistance.ampId', e.target.value)} readOnly={!isEditing} className="report-input" />
          </div>
          <div className="equipment-row">
            <label>Hi-Pot:</label>
            <input value={formData.testEquipment?.hipot?.name || ''} onChange={e => setField('testEquipment.hipot.name', e.target.value)} readOnly={!isEditing} className="report-input" placeholder="Name" />
            <label>Serial #:</label>
            <input value={formData.testEquipment?.hipot?.serialNumber || ''} onChange={e => setField('testEquipment.hipot.serialNumber', e.target.value)} readOnly={!isEditing} className="report-input" />
            <label>AMP ID:</label>
            <input value={formData.testEquipment?.hipot?.ampId || ''} onChange={e => setField('testEquipment.hipot.ampId', e.target.value)} readOnly={!isEditing} className="report-input" />
          </div>
        </div>
      </section>
